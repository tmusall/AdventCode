...
class Trench {
  public:
    ...
    bool shiftColumns() {
      ...
      Row first_row(*rows[0]);
      Row this_row(first_row);
      Row next_row(first_row);
      ...
      for (auto iter=rows.begin(); iter != rows.end(); iter++)
      {
        ...
        if (iter_next == rows.end())
        {
          next_row = first_row;
          iter_next = rows.begin();
        }
        else
        {
          next_row = *(*iter_next);
        ...
        {
          char c = this_row.GetChar(col);
          if (c == 'v')
          {
            if (next_row.GetChar(col) == '.')
            {
              (*iter_next)->SetChar(col, c);
              (*iter)->SetChar(col, '.');
              didMove = true;
            }
          }
    ...
    bool shiftColumns_better() {
      bool didMove = false;
      size_t numCols = rows[0]->GetLength();

      for (size_t col=0; col<numCols; col++)
      {
        bool bCanWrap = rows[0]->GetChar(col) == '.';

        for (auto r=rows.begin(); r != rows.end(); r++)
        {
          char c = (*r)->GetChar(col);
          if (c == 'v')
          {
            auto next_row = r + 1;
...
int main() {
  ...
  // Trench trench("sample_input.txt");
